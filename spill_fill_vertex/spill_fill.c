...
int main() {
    ...
    // NOTE: Make sure the file name here matches what you output from glslangValidator!
    FILE *f = fopen("vert.spv", "rb");
    if (!f) {
        printf("[ERROR] Could not open vert.spv. Did you compile the vertex shader?\n");
        printf("Run: glslangValidator -V shader.vert -o vert.spv\n");
        return 1;
    }
    fseek(f, 0, SEEK_END);
    ...
    VkInstance instance;
    VkApplicationInfo appInfo = { VK_STRUCTURE_TYPE_APPLICATION_INFO, NULL, "SpillTest", 1, "NoEngine", 1, VK_API_VERSION_1_0 };
    ...
    // 5. Render Pass (Vulkan Graphics Pipelines mandate a Render Pass)
    ...
    VkRenderPassCreateInfo rpInfo = { 
        ...
        0, 
        0, NULL, 
        1, &subpass, // Fixes the crash by ensuring at least 1 subpass exists
        0, NULL 
    };
    VkRenderPass renderPass;
    CHECK_VK(vkCreateRenderPass(device, &rpInfo, NULL, &renderPass));

    VkPipelineShaderStageCreateInfo shaderStage = { VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, NULL, 0, VK_SHADER_STAGE_VERTEX_BIT, shaderModule, "main", NULL };
    VkPipelineVertexInputStateCreateInfo vertexInput = { VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO, NULL, 0, 0, NULL, 0, NULL };
    VkPipelineInputAssemblyStateCreateInfo inputAssembly = { VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO, NULL, 0, VK_PRIMITIVE_TOPOLOGY_POINT_LIST, VK_FALSE };
    ...
    VkPipelineRasterizationStateCreateInfo rasterization = { VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO, NULL, 0, VK_FALSE, VK_FALSE, VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE, VK_FRONT_FACE_CLOCKWISE, VK_FALSE, 0, 0, 0, 1.0f };
    ...
    VkPipeline pipeline;
    CHECK_VK(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, NULL, &pipeline));
    ...
    // Minimal dummy Framebuffer
    VkFramebufferCreateInfo fbInfo = { VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO, NULL, 0, renderPass, 0, NULL, 1, 1, 1 };
    VkFramebuffer framebuffer;
    CHECK_VK(vkCreateFramebuffer(device, &fbInfo, NULL, &framebuffer));

    VkRenderPassBeginInfo rpBegin = { VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO, NULL, renderPass, framebuffer, {{0, 0}, {1, 1}}, 0, NULL };
    ...
    // Memory Barrier to read back operations safely
    ...
    vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 0, NULL, 1, &barrier, 0, NULL);
